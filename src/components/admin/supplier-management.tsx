...
import { CreateSupplierInput, UpdateSupplierInput } from "@/types/forms/supplier-form";
...
export function SupplierManagement() {
  ...
  async function loadSuppliers() {
    ...
    const response = await fetch(`/api/admin/suppliers${query}`, { cache: "no-store" });
    const body = (await response.json().catch(() => null)) as ApiSuccess<Supplier[]> | ApiError | null;

    if (!response.ok) {
      setError((body as ApiError)?.message ?? "Gagal memuat data supplier");
      setLoading(false);
      return;
    }

    setSuppliers((body as ApiSuccess<Supplier[]>)?.data ?? []);
    setLoading(false);
  }
  ...
  async function createSupplier(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) {
      setError((body as ApiError)?.message ?? "Gagal menambah supplier");
      return;
    }
  ...
  async function submitEdit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/admin/suppliers/${editingSupplierId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    if (!response.ok) {
      setError((body as ApiError)?.message ?? "Gagal update supplier");
      return;
    }
    ...
    setForm(initialForm);
    setIsModalOpen(false);
    setMessage("Data supplier berhasil diperbarui.");
    await loadSuppliers();
  }
  ...
  async function deleteSupplier(supplierId: string) {
    ...
    if (!response.ok) {
      setError((body as ApiError)?.message ?? "Gagal menghapus supplier");
      setDeleting(false);
    ...
    <section className="flex flex-col gap-4">
        ...
        <CardHeader className="space-y-3">
          <ManagementToolbar
            ...
            placeholder="Cari nama / kode / telepon / kontak supplier"
          />
        </CardHeader>
        <CardContent className="space-y-3">
          {loading ? <ManagementStatusMessage type="loading">Memuat data supplier...</ManagementStatusMessage> : null}
          ...
          {!loading && filteredSuppliers.length === 0 ? (
            <ManagementEmptyState title="Belum ada data supplier." />
          ) : null}

          {viewMode === 'grid' ? (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {filteredSuppliers.map((supplier) => (
                  <div key={supplier.id} className="group relative overflow-hidden rounded-2xl border border-border/50 bg-white p-4 transition-all hover:border-primary/30 hover:shadow-sm">
                      <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                              ...
                              <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                                  Kode: {supplier.code || "-"} | CP: {supplier.contact_person || "-"}
                              </p>
                          ...
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              <Button size="icon" variant="ghost" className="h-8 w-8 rounded-full text-primary" onClick={() => startEdit(supplier)}>
            ...
            <div className="space-y-2">
                {filteredSuppliers.map((supplier) => (
                    <div key={supplier.id} className="group relative overflow-hidden rounded-2xl border border-border/50 bg-white p-3 transition-all hover:border-primary/30 hover:shadow-sm">
                        <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-sm font-black text-foreground uppercase tracking-tight">{supplier.name}</p>
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{supplier.code || "-"} | {supplier.phone || "-"}</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <Button size="sm" variant="ghost" className="h-8 text-[10px] font-bold uppercase text-primary hover:bg-primary/5" onClick={() => startEdit(supplier)}>Edit</Button>
                                <Button 
                                    size="sm" 
                                    variant="ghost" 
      ...
      >
            <Card className="w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200">
                ...
                <CardContent>
                    <form className="space-y-4" onSubmit={editingSupplierId ? submitEdit : createSupplier}>
      ...
      <ConfirmActionDialog
        ...
        description="Apakah Anda yakin ingin menghapus supplier ini? Data supplier yang sudah dihapus tidak dapat dikembalikan."
        confirmLabel="Hapus"
        variant="destructive"
        loading={deleting}
        onConfirm={() => supplierToDelete && deleteSupplier(supplierToDelete)}
      />
    </section>
  );
}
